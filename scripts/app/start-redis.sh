...
echo ""
echo "==> Starting preventia-redis container..."

if podman ps -a --format '{{.Names}}' | grep -q '^preventia-redis$'; then
  STATE=$(podman inspect --format '{{.State.Status}}' preventia-redis)
  if [[ "$STATE" == "running" ]]; then
    echo "    Container already running — nothing to do."
  else
    echo "    Container exists but is stopped — starting it..."
    podman start preventia-redis
  fi
else
  podman run -d \
    --name preventia-redis \
