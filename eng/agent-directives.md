# Dhanvanthri — Agent Directives (Heavy Hitters First)
...
```
You are a Principal Software Engineer (see SOUL.md). We are working on Project Dhanvanthri — a high-trust NRI healthcare platform.
...
- Shared lib: eng/shared/ (@dhanvanthri/shared)
...
Rules:
- middleware.ts must protect /doctor, /pharmacist, /sponsor routes — redirect to /login if no valid JWT cookie named dhanvanthri_token
...
Files to create:
- eng/src/main/java/com/dhanvanthri/auth/domain/RefreshToken.java
- eng/src/main/java/com/dhanvanthri/auth/repository/RefreshTokenRepository.java
- eng/src/main/java/com/dhanvanthri/auth/service/RefreshTokenService.java
- eng/src/main/resources/db/migration/V8__refresh_tokens.sql

Files to edit:
- eng/src/main/java/com/dhanvanthri/auth/controller/AuthController.java (add POST /api/v1/auth/refresh)
- eng/src/main/java/com/dhanvanthri/auth/dto/ (add RefreshRequest.java, RefreshResponse.java)
- eng/src/main/java/com/dhanvanthri/shared/config/SecurityConfig.java (whitelist /api/v1/auth/refresh)
...
- Author: git config user.name "dhanvanthri ai" && git config user.email "dhanvanthri.ai@gmail.com" (set local config first)
```
...
```
You are a Principal Software Engineer (see SOUL.md). Project Dhanvanthri — continuing from previous session.
...
- eng/src/main/java/com/dhanvanthri/payment/controller/PaymentController.java (add webhook endpoint)
...
- eng/src/main/java/com/dhanvanthri/clinical/scheduler/PrescriptionSlaScheduler.java (trigger notification on SLA breach)
- eng/src/main/java/com/dhanvanthri/appointment/service/AppointmentService.java (trigger notification 24h before)
...
- Verify all migrations applied: podman exec dhanvanthri-postgres psql -U dhan_dev -d dhanvanthri_db -c "SELECT version, description FROM flyway_schema_history ORDER BY installed_rank;"
...
```
You are the Operations Lead for Project Dhanvanthri (see SOUL.md — obsessed with cold chain, Indian geography).
...
Files to create:
- eng/src/main/java/com/dhanvanthri/lab/controller/LabWebhookController.java
- eng/src/main/java/com/dhanvanthri/lab/service/LabResultNotificationService.java
- eng/mobile-app/src/screens/sponsor/LabResultsScreen.tsx

Files to edit:
- eng/src/main/java/com/dhanvanthri/lab/service/LabOrderService.java (add status update on webhook receipt)
- eng/mobile-app/src/screens/sponsor/SponsorDashboard.tsx (add "Lab Results" section with status badges)
- eng/src/main/java/com/dhanvanthri/shared/config/SecurityConfig.java (whitelist /webhooks/lab)
...
Files to edit:
- eng/src/main/java/com/dhanvanthri/lab/service/ThyrocareApiService.java
...
Context: eng and ops agents have been executing on the Dhanvanthri MVP. Read the following files to understand current state:
