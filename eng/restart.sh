...
# ── 3. Stop & remove existing Dhanvanthri containers ────────
echo "\n🛑  Stopping existing containers..."
$COMPOSE_CMD down --remove-orphans 2>/dev/null || true

# Also clean up any manually-started redis container
podman rm -f dhanvanthri-redis 2>/dev/null || true
...
for i in {1..20}; do
  STATUS=$(podman inspect --format='{{.State.Health.Status}}' dhanvanthri-postgres 2>/dev/null || echo "missing")
